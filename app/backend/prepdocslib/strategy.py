...
from .search_config import get_search_api_version
...
class SearchInfo:
    ...
    def __init__(
        ...
        self.api_version = api_version or get_search_api_version()
    ...
    def create_search_index_client(self) -> SearchIndexClient:
        return SearchIndexClient(endpoint=self.endpoint, credential=self.credential, api_version=self.api_version)

    def create_search_indexer_client(self) -> SearchIndexerClient:
        return SearchIndexerClient(endpoint=self.endpoint, credential=self.credential, api_version=self.api_version)
